...
/**
 * Test the generated proxies behavior. These tests make assumptions about the structure of LazyLoadableObjectWithReadonlyPublicProperties
 */
class ProxyLogicReadonlyPublicPropertiesTest extends TestCase
{
    /** @var MockObject&RProxyLoader */
    protected $proxyLoader;

    /** @var ClassMetadata */
    protected $lazyLoadableObjectMetadata;

    /** @var LazyLoadableObjectWithReadonlyPublicProperties&Proxy */
    ...
    {
        $loader                           = $this->proxyLoader      = $this->createMock(RProxyLoader::class);
        $this->initializerCallbackMock    = $this->getMockBuilder(stdClass::class)->setMethods(['__invoke'])->getMock();
        $identifier                       = $this->identifier;
        $this->lazyLoadableObjectMetadata = $metadata = new LazyLoadableObjectWithReadonlyPublicPropertiesClassMetadata();

        // emulating what should happen in a proxy factory
        $cloner = static function (LazyLoadableObjectWithReadonlyPublicProperties $proxy) use ($loader, $identifier, $metadata) {
            /** @var LazyLoadableObjectWithReadonlyPublicProperties&Proxy $proxy */
            $proxy = $proxy;
            ...
            foreach ($metadata->getReflectionClass()->getProperties() as $reflProperty) {
                ...
                if (in_array($propertyName, $identifierFields) || (! $metadata->hasField($propertyName) && ! $metadata->hasAssociation($propertyName)) ) {
                    continue;
                }
        ...
        $proxyClassName = 'Doctrine\Tests\Common\ProxyProxy\__CG__\Doctrine\Tests\Common\Proxy\LazyLoadableObjectWithReadonlyPublicProperties';
    ...
    public function testCloningCallsClonerWithClonedObject()
    {
        ...
        $test       = $this;
        $callback   = static function (LazyLoadableObjectWithReadonlyPublicProperties $proxy) use ($lazyObject, $test) {
            assert($proxy instanceof Proxy);
            $test->assertNotSame($proxy, $lazyObject);
            $proxy->__setInitializer(null);
            $proxy->publicAssociation = 'clonedAssociation';
        };
        $cb         = $this->createMock(RCloner::class);
        $cb
            ->expects($this->once())
            ->method('cb')
    ...
    public function testLoadingWithPersisterWillBeTriggeredOnlyOnce()
    {
        $this
            ...
            ->with(
                ...
                ->will($this->returnCallback(static function ($id, LazyLoadableObjectWithReadonlyPublicProperties $lazyObject) {
        ...
        $this
            ...
            ->with([
                ...
                ->will($this->returnCallback(static function () {
                    $blueprint = new LazyLoadableObjectWithReadonlyPublicProperties(
    ...
    public function testNotInitializedProxyUnserialization()
    {
        ...
        /** @var LazyLoadableObjectWithReadonlyPublicProperties&Proxy $unserialized */
    ...
    public function testInitializedProxyUnserialization()
    {
        ...
        $this
            ...
            ->will($this->returnCallback(static function ($id, LazyLoadableObjectWithReadonlyPublicProperties $lazyObject) use($test) {
                // setting a value to verify that the persister can actually set something in the object
                $test->setProxyValue('publicPersistentField', 'publicPersistentFieldValue');
                $test->setProxyValue('publicAssociation', 'publicAssociationValue');
                return true;
            }));
    ...
    protected function getSuggestedInitializerImplementation()
    {
        ...
        return static function (LazyLoadableObjectWithReadonlyPublicProperties $proxy) use ($loader, $identifier) {
            /** @var LazyLoadableObjectWithReadonlyPublicProperties&Proxy $proxy */
            $proxy = $proxy;
            $proxy->__setInitializer(null);
